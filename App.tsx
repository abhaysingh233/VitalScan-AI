...
import { Search, RotateCcw, Info, Menu, X, BrainCircuit, Heart, Activity, User, MapPin, ClipboardList, Shield, Settings, LogOut, ChevronRight, Sparkles, Mic, MicOff, Sun, Moon, ExternalLink, Navigation, ScanEye, Upload, ImageIcon, AlertTriangle, Globe, Smartphone, Watch, Wifi, ShieldCheck, Lock, Download, Star, Volume2, Award, Stethoscope, FileSignature, CheckCircle2, MessageSquare, Send, Camera, SwitchCamera, BarChart3, Moon as MoonIcon, Scale, Zap, Trophy } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Cleanup to prevent ghost listeners when switching tabs
    return () => {
  ...
  return (
    <div className="app-viewport bg-[#fcfdff] dark:bg-slate-950 selection:bg-sky-100 dark:selection:bg-sky-900 selection:text-sky-900 dark:selection:text-sky-100 transition-colors duration-300">
      ...
      {/* Grid Layout */}
      <main className="flex-1 w-full max-w-screen-2xl mx-auto p-4 sm:p-8 lg:p-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start h-full">
          ...
          {/* Rest of the component follows unchanged logic... */}
                ...
                <div className="bg-white dark:bg-slate-900 p-12 sm:p-16 rounded-[4.5rem] border border-slate-100 dark:border-slate-800 shadow-2xl flex flex-col md:flex-row items-center gap-16 relative overflow-hidden group transition-colors">
                  <div className="flex-1 text-center md:text-left space-y-6">
                    ...
                    <button 
                        ...
                        className="px-6 py-3 bg-slate-100 dark:bg-slate-800 text-rose-500 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-rose-50 dark:hover:bg-rose-900/20"
                    >
                        Terminate Session
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                     ...
                     <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm">
                        <div className="flex items-center gap-2 mb-2 text-indigo-500">
                           <MoonIcon size={18} fill="currentColor" />
                           <span className="text-[10px] font-black uppercase tracking-widest">Sleep</span>
                        </div>
                        <input 
                           type="number" 
                           value={sleepHours} 
                     ...
                     <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm">
                        <div className="flex items-center gap-2 mb-2 text-emerald-500">
                           <Scale size={18} />
                           <span className="text-[10px] font-black uppercase tracking-widest">BMI</span>
                        </div>
                        <div className="text-3xl font-black text-slate-900 dark:text-white">{bmi}</div>
                        <div className="flex gap-2 text-[9px]">
                           <input type="number" value={weight} onChange={(e) => setWeight(Number(e.target.value))} className="w-12 bg-slate-50 dark:bg-slate-800 rounded px-1" /> kg
                           <input type="number" value={height} onChange={(e) => setHeight(Number(e.target.value))} className="w-12 bg-slate-50 dark:bg-slate-800 rounded px-1" /> cm
                        </div>
                     </div>
                     <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                >
                                    ...
                                    <div className="relative z-10">
                                        ...
                                        {/* Signature Footer */}
                                        <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-700/50 mt-6">
