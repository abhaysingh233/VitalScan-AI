...
import { Activity, ShieldCheck, HeartPulse, Pill, Stethoscope, AlertTriangle, FileDown, Clock, Sparkles, ChevronRight, CheckCircle2, Dna, Microscope } from 'lucide-react';
    ...
    <div className="space-y-12 main-content animate-in fade-in slide-in-from-bottom-4 duration-700 overflow-visible">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 no-print stagger-in" style={{ animationDelay: '0.1s' }}>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
          <div className="w-full sm:flex-1 lg:flex-none flex items-center justify-center gap-3 px-6 py-4 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white rounded-[1.5rem] border border-slate-200 dark:border-slate-700 font-black text-xs uppercase tracking-widest shadow-sm transition-colors">
            <Stethoscope size={18} className="text-sky-500" />
            {data.specialist}
          </div>
          <button 
            onClick={handleDownloadPDF}
            className="w-full sm:w-auto flex items-center justify-center gap-3 px-6 py-4 bg-slate-900 dark:bg-slate-700 text-white rounded-[1.5rem] text-xs font-black uppercase tracking-widest hover:bg-slate-800 dark:hover:bg-slate-600 hover:shadow-2xl hover:shadow-slate-200 dark:hover:shadow-none transition-all duration-300 active:scale-95 group"
          >
      ...
      {/* Biological Context (New Section) */}
      ...
      {/* Pharmaceutical Registry - Fixed Overlaps */}
      <div className="bg-white dark:bg-slate-900 p-8 md:p-12 rounded-[3rem] md:rounded-[4rem] shadow-2xl shadow-slate-200/20 dark:shadow-none border border-slate-100 dark:border-slate-800 card relative overflow-hidden stagger-in transition-colors" style={{ animationDelay: '0.4s' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {data.medicines.map((m, i) => (
            <div key={i} className="flex flex-col p-6 md:p-8 bg-slate-50/50 dark:bg-slate-800/50 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 hover:border-rose-100 dark:hover:border-rose-900 hover:bg-white dark:hover:bg-slate-800 transition-all duration-300 shadow-sm group min-h-[160px]">
              <span className="font-black text-slate-900 dark:text-white block mb-4 text-lg md:text-xl tracking-tighter group-hover:text-rose-600 dark:group-hover:text-rose-400 transition-colors uppercase break-words hyphens-auto leading-none">
                {m.name}
              </span>
              <p className="text-[10px] md:text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed font-bold italic opacity-80 uppercase tracking-wider">
                {m.warning}
              </p>
            </div>
          ))}
